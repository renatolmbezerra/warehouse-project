...
CTE_ComprasCliente AS (
    ...
    SELECT
        A.CODCFO,
        SA.TIPOS AS TIPOFAM,
        CAST(A.VALORFINANCEIRO AS NUMERIC(18, 2)) AS VLR_ULT_COMPRA,
        SUM(CAST(A.VALORFINANCEIRO AS NUMERIC(18, 2))) OVER(PARTITION BY A.CODCFO) AS VALOR_FAT,
        MIN(A.DATAEMISSAO) OVER(PARTITION BY A.CODCFO) AS PRIMEIRA_COMPRA,
        MAX(A.DATAEMISSAO) OVER(PARTITION BY A.CODCFO) AS ULTIMA_COMPRA,
        ROW_NUMBER() OVER(PARTITION BY A.CODCFO ORDER BY A.DATAEMISSAO DESC) AS RN
    FROM {{ ref('gld_fct_vendas_itens') }} A
...
CTE_Maior_Compra AS (
    ...
    FROM (
        SELECT
            CODCFO,
            IDMOV,
            DATAEMISSAO AS DATA_MAIOR_COMPRA,
            CODTMV,
            SUM(VALORFINANCEIRO) AS MAIOR_COMPRA
        FROM {{ ref('gld_fct_vendas_itens') }}
        GROUP BY IDMOV, CODCFO, CODTMV, DATAEMISSAO
    ) FAT
),
...
CTE_LTM AS (
    SELECT 
        CODCFO,
        SUM(CAST(VALORFINANCEIRO AS NUMERIC(18,2))) AS LTM_VLR,
        COUNT(DISTINCT IDMOV) AS LTM_QTD_VEN,
        SUM(CAST(VALORFINANCEIRO AS NUMERIC(18,2))) / NULLIF(COUNT(DISTINCT IDMOV), 0) AS TICKET_MED_LTM
    FROM {{ ref('gld_fct_vendas_itens') }}
    WHERE 
        DATAEMISSAO BETWEEN (CURRENT_DATE - INTERVAL '1' YEAR) AND CURRENT_DATE
    GROUP BY CODCFO
),
...
CTE_Primeiro_Vendedor AS (
    SELECT * FROM (
        SELECT 
            ...
            IDMOV,
            DATAEMISSAO AS DATA,
            ROW_NUMBER() OVER (PARTITION BY CODCFO ORDER BY DATAEMISSAO ASC, IDMOV ASC) AS RN_PRI
        FROM {{ ref('gld_fct_vendas_itens') }}
    ) Pri_Vendedor
    WHERE RN_PRI = 1
),
...
CTE_Ultimo_Vendedor AS (
    SELECT * FROM (
        SELECT 
            ...
            IDMOV,
            DATAEMISSAO AS DATA,
            ROW_NUMBER() OVER (PARTITION BY CODCFO ORDER BY DATAEMISSAO DESC, IDMOV DESC) AS RN_ULT
        FROM {{ ref('gld_fct_vendas_itens') }}
    ) Ult_Vendedor
    WHERE RN_ULT = 1
),
